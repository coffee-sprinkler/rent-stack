'use client';
// app/(dashboard)/dashboard/maintenance/MaintenanceClient.tsx
...
function MaintenanceRow({ request }: { request: Request }) {
  ...
  return (
    <tr className='border-b border-zinc-800 last:border-0 hover:bg-zinc-800/50 transition'>
      ...
      <td className='px-6 py-4 hidden lg:table-cell'>
        <p className='text-white'>Unit {request.unit.unit_number}</p>
        <p className='text-xs text-zinc-500'>{request.unit.property.name}</p>
      </td>
      <td className='px-6 py-4'>
        <span
          className={`text-xs px-2.5 py-1 rounded-full border capitalize ${PRIORITY_STYLES[request.priority]}`}
        >
          {request.priority}
        </span>
      </td>
      <td className='px-6 py-4'>
        <span
          className={`text-xs px-2.5 py-1 rounded-full border ${STATUS_STYLES[status]}`}
        >
          {status.replace('_', ' ')}
        </span>
      </td>
        ...
        {status !== 'resolved' && (
          <div className='flex flex-col items-end gap-1'>
            <select
              ...
              disabled={isPending}
              className='text-xs bg-zinc-800 border border-zinc-700 rounded-lg px-2 py-1.5 text-white focus:outline-none focus:border-indigo-500 transition disabled:opacity-40'
            >
      ...
      {requests.length === 0 ? (
        ...
        <div className='bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden'>
          <table className='w-full text-sm'>
            ...
            <tbody>
              {requests.map((r) => (
                <MaintenanceRow key={r.id} request={r} />
              ))}
            </tbody>
          </table>
        </div>
